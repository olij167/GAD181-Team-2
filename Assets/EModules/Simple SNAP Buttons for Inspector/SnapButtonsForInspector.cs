...
namespace EModules.Snap
{
    ...
    class Snap
    {
        ...
        static Snap() {
            ...
            var snapSettings = typeof( Editor ).Assembly.GetType( "UnityEditor.SnapSettings" );
        ...
        static void EditorApplication_UPDATESNAPPING() {
            ...
            foreach ( var t in Selection.GetTransforms( SelectionMode.TopLevel | SelectionMode.Editable ) ) {
        ...
        private static void SceneView_PLACEONSURFACE( SceneView sceneView ) {
            ...
            if ( Event.current.type == EventType.MouseDown && (Selection.GetTransforms( SelectionMode.TopLevel | SelectionMode.Editable ).Length > 0) ) {   /* if ( PLACE_ON_SURFACE_ALIGNBYMOUSE )
                         mouseTunning = new[] { (HandleUtility.WorldToGUIPoint( Tools.handlePosition ) - Event.current.mousePosition) };
                     else*/
                mouseTunning = Selection.GetTransforms( SelectionMode.TopLevel | SelectionMode.Editable ).Select( t => HandleUtility.WorldToGUIPoint( t.position ) - Event.current.mousePosition ).ToArray();
            }
    ...
    public class SnapButtonsForInspectorGUI : Editor
    {
        ...
        void InitStyles() {   /*var s = ScriptableObject.CreateInstance<SnapButtonsForInspector>();
